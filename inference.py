...
class PredOptions():
    ...
    def initialize(self, parser):
        ...
        parser.add_argument('--test_dir', required=False, default=r'.../SECOND/test', help='directory to test images')
...
def main():
    ...
    compatible_missing_keys = [
        "CD_Decoder.clip_injector.logit_scale",
        "CD_Decoder.clip_injector.attn_temp",
    ]

    for key in compatible_missing_keys:
        if key not in new_ckpt:
            print(f"[Checkpoint compatibility] Initialize missing key: {key}")
            new_ckpt[key] = model_state[key].clone()
