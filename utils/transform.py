...
def rand_rot90_flip_SCD(img1, img2, lb1, lb2):
    """
    随机旋转和翻转图像及标签，用于SCD（语义变化检测）任务。
    保证 im1, im2, label1, label2 做完全相同的变换。
    """
    # 1. 随机旋转 (0, 90, 180, 270 度)
    k = random.randint(0, 3) 
    # rot90 默认处理前两个轴 (H, W)，这对 HWC 格式的图片是正确的
    img1 = np.rot90(img1, k).copy()
    img2 = np.rot90(img2, k).copy()
    lb1 = np.rot90(lb1, k).copy()
    lb2 = np.rot90(lb2, k).copy()

    # 2. 随机垂直翻转
    if random.random() > 0.5:
        img1 = np.flipud(img1).copy()
        img2 = np.flipud(img2).copy()
        lb1 = np.flipud(lb1).copy()
        lb2 = np.flipud(lb2).copy()

    # 3. 随机水平翻转
    if random.random() > 0.5:
        img1 = np.fliplr(img1).copy()
        img2 = np.fliplr(img2).copy()
        lb1 = np.fliplr(lb1).copy()
        lb2 = np.fliplr(lb2).copy()
        
    return img1, img2, lb1, lb2
