...
def SCDD_eval_all(preds, labels, num_class):
    ...
    union = c2hist.sum(1) + c2hist.sum(0) - np.diag(c2hist)
    iu = np.diag(c2hist) / (union + 1e-6)
    ...
    SC_Precision = SC_TP / (change_pred_sum + 1e-6)
    SC_Recall = SC_TP / (change_label_sum + 1e-6)
    
    Fscd = 2 * SC_Precision * SC_Recall / (SC_Precision + SC_Recall + 1e-6)
